...
搜了下，这个东西叫`FrontPage Server Extensions`,  相关搜索比较少

[http://10.10.10.14/_vti_bin/_vti_adm/fpadmdll.dll](http://10.10.10.14/_vti_bin/_vti_adm/fpadmdll.dll)，有个401认证, 尝试弱密码，失败

又想起[这是一篇“不一样”的真实渗透测试案例分析文章](https://paper.seebug.org/1144/)里说`webbdav`好像是有个`xxe`，在根目录上尝试，无果

msf打了了`iis 6.0`+`sharepoint`+`webdav`的exp，无果，用`davtest`扫描`webdav`是否有漏洞，全部失败。
...
苦苦搜寻，几番尝试无果后，准备看网上的walkthrough，居然是用msf中我尝试过的一个exp直接rce了，
...
# 提权
使用msf自带的一个提权测试模块，`**post/multi/recon/**``**local_exploit_suggester**`**  **
...
找个可写目录, 再反弹一个`shell`，以下exp均可成功提权

+ `windows/local/ms14_058_track_popup_menu`
+ `exploit/windows/local/ms14_070_tcpip_ioctl` 稳定
