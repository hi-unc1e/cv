...
(1)注意到`xmlrpc.php`可以爆破，于是使用wpscan，对top1w的爆破，失败
...
尝试构造参数，怎么都不成功。。![](https://cdn.nlark.com/yuque/0/2021/png/166008/1621525207493-33b55114-4cae-4baa-9fd3-85398d89e6be.png)



又兜兜转转找了一个多小时，从手动尝试到[SecLists/Fuzzing/LFI](https://github.com/danielmiessler/SecLists/tree/285474cf9bff85f3323c5a1ae436f78acd1cb62c/Fuzzing/LFI)，终于有所突破。![](https://cdn.nlark.com/yuque/0/2021/png/166008/1621525374524-bcfda26c-1de5-4fce-890b-42d01acfbf5b.png)
...
用刚刚收集的密码直接进去了，OSINT到[https://www.exploit-db.com/exploits/49810](https://www.exploit-db.com/exploits/49810)
...
端口转发完全失败，我真的疯了，直到我想起它——python，简单测试了一下，居然没有报错！那就用`request`来打！
...
利用参考[它](https://blog.pentesteracademy.com/abusing-sys-module-capability-to-perform-docker-container-breakout-cf5c29956edd)
...
+ **目录扫描时，应该对**`**/wp-content/plugins/wp-with-spritz/**`**进行递归扫描的？**

其实，扫目录为什么会找到`wp-with-spritz`呢，毕竟字典`DirBuster\directory-list-lowercase-2.3-medium.txt`里面并没有`spritz`这个关键词啊。原来，是目录遍历！
...
1. apache2的默认配置目录：`/etc/apache2/sites-available/000-default.conf`
...
<font style="color:rgb(36, 39, 41);">一样的。</font>
...
跟rsa类似的一种非对称加密方式，可以使用`ssh-keygen -t dsa`来生成密钥，生成后的结果如图所示：
